...
int Game::showTime(const Point& legend, bool silentMode,bool reset = false)
{
	static int secs;
	if (reset)
	{
...
bool Game::barrelsUpdateDirs(vector<Barrel>* barrels, char board[][GameConfig::WIDTH - 2], Player* mario,bool silentMode)
{
	for (int i = 0;i < barrels->size();i++)
	{
		...
		case GameConfig::ARROWKEYS::DOWNANDLEFT:
		{
			...
			else if (barreldistfloor == GameConfig::FLOORDIFF - 1&&!silentMode)
...
void Game::ghostsChangeDir(vector<Ghost*> ghosts, char board[][GameConfig::WIDTH - 2], Ladder lad[], int size,bool silentMode)
{
...
void Game::nextLevelLoadMode(const std::map<int, Level>& map,std::map<int, Level>::iterator& currPos,ifstream& stepsInput,ifstream& resultInput)
{
	if (currPos == map.end())
		return ;
	...
	getFilename(currPos->first, stepsfilename, resultfilename);
	stepsInput.open(stepsfilename,ios::in);
	resultInput.open(resultfilename,ios::in);

	while ((!stepsInput.is_open() || !resultInput.is_open()) && currPos!=map.end())
	{
		std::cerr << "Error: Could not found Level " << currPos->first <<", Press any key to look for the next one" << std::endl;
		_getch();
		currPos++;
		stepsfilename.clear();
	...
	if (currPos != map.end())
	{
		stepsInput.clear();               // Clear any error flags
		stepsInput.seekg(0, ios::beg);    // Move back to the beginning
		resultInput.clear();               // Clear any error flags
		resultInput.seekg(0, ios::beg);    // Move back to the beginning
	}

}
bool Game::checkLifeLostPoint(int numlives, int ptOftime, const Result& resrecords)
{
...
void Game::run(bool saveMode, bool loadMode, bool silentMode)
{
	...
	Level currLvl;
	int gameinterval;
	loadMode ||silentMode ? gameinterval = LOADINTERVAL : gameinterval = INTERVAL;

	map<int, Level> alllevels;
	gameValid = FileHandler::loadAllFiles(alllevels);
	auto currMapIndex = alllevels.begin();
	...
	std::ifstream InputstepsFile;
	std::ifstream InputresultsFile;
	...
	while (!exit) {
		...
		{
			nextLevelLoadMode(alllevels, currMapIndex, InputstepsFile, InputresultsFile);
			if (currMapIndex == alllevels.end())
			{
				cout << "Not found a valid record file" << endl;
				gameRunning = false;
				exit = true;
			}
			else
		...
		while (gameRunning) {
			...
			{ 
				recordedSteps.clear();
				currentResult.lives.clear();
				currentResult.livesfinished = currentResult.scorefinished = currentResult.timefinished = 0;

				// Read the seed from the first line.
				std::string line;
				if (std::getline(InputstepsFile, line))
				{
					std::istringstream iss(line);
					iss >> loadedSeed;
					std::srand(loadedSeed);
				}
				while (std::getline(InputstepsFile, line))
				{
				...
				while (std::getline(InputresultsFile, line) && FileHandler::starts_with(line,"Lives:"))
				{
					std::string records = line.substr(6);
					std::istringstream iss(records);
					int pointOfTime,numLives;
					
					iss >> numLives >> pointOfTime;
					currentResult.lives.push_back({ numLives,pointOfTime });
				}
				if (FileHandler::starts_with(line, "Score:"))
				{
					std::string records = line.substr(6);
					if(records.size()<3) //Score is F (filure)
						currentResult.passed = false;
					else //Passed the level
					{
						std::istringstream iss(records);
						iss >> currentResult.scorefinished >> currentResult.livesfinished >> currentResult.timefinished;
					}
				}
				else //Game exited in the middle
				{
					currentResult.passed = false;
				}

				InputresultsFile.close();
				InputstepsFile.close();
			...
			do {
				...
				char prevKeyPressed = GameConfig::ARROWKEYS::STAY; //useful for load mode only
				...
				if (loadMode||silentMode)
				{
					...
					if (recordedIndex < recordedSteps.size() && recordedSteps[recordedIndex].timeStep == gameCounter+1)
					{
						keyPressed = static_cast<char>(recordedSteps[recordedIndex].move);
						if (keyPressed == GameConfig::UPANDRIGHT || keyPressed == GameConfig::UPANDLEFT)
							keyPressed = GameConfig::UP;
					...
					else
					{
						...
						else //Not valid time steps
						{
							...
							cout << "Missing a record point " << gameCounter + 1;
							escPressed = true;
							exit = true;
							break;
						}
					...
					if (!wPressed) //Not Jump Mode
					{
						...
						if (((loadMode||silentMode) &&(prevKeyPressed!=keyPressed)&&descent==0) ||(!(loadMode||silentMode)&&(_kbhit() && descent == 0))) //Confirms the player is not falling down while hitting the button
						{
							if(!(loadMode||silentMode))
							keyPressed = _getch();
							...
							case 'a':
							case  'A':
								if ((!(loadMode||silentMode)&& climb == 0)||((loadMode || silentMode)&&climb<2))
								{
									if (loadMode || silentMode)
										climb = 0;
				...
				ghostsChangeDir(activeghosts, currLevel->getBoardPointer(), currLevel->getLadders(), currLevel->getNumLadders(),silentMode);
				...
				{
					timePlayed -= SECOND;
					showTime(currLevel->getLegendPos(),silentMode);
				}
			...
			if (escPressed == false)
			{
				if (loadMode || silentMode)
				{
					if (!checkResults(score, gameCounter, lives, finished, currentResult,recordedSteps))
					{

						gotoxy(0, GameConfig::MIN_Y + GameConfig::HEIGHT + 1);
						cout << "in result file, the stats wrong";
						_getch();
						exit = true;
						break;
					}
				...
				if (finished)
				{
					...
					if (saveMode)
					{
						result << "Score: " << score << " " << lives << " " << gameCounter;
						result.close();
					}
					if (!silentMode)
					{
					...
					if (currMapIndex != alllevels.end())
					{
						...
						{
							nextLevelLoadMode(alllevels, currMapIndex, InputstepsFile, InputresultsFile);
							if (currMapIndex == alllevels.end()) //No more valid steps and result files found
							{
								gotoxy(0, GameConfig::HEIGHT + GameConfig::MIN_Y + 1);
								cout << "Failure";
								exit = true;
					...
					else //All levels finished
					{
						...
						{
							int gamesecs = showTime(currLevel->getLegendPos(),silentMode ,true);
				...
				else
				{
					...
					else
					{
						...
						gotoxy(0, GameConfig::HEIGHT + GameConfig::MIN_Y + 1);
						std::cout << "Failure,";
						showTime(currLevel->getLegendPos(),silentMode ,true);
						gameRunning = false;
						std::cout << "Press any key to continue";
						_getch();
					}
